...
//			initialValues.put(MUSIC_HISTORY_FINGERPRINT, row.getFingerprintData());
		...
		try {
			...
			while(iter.hasNext()){
				...
				byte[] imageData = getDataFromHttpGetURL("http://"+coverArtUrl);
				if(imageData != null){
	...
	static byte[] getDataFromHttpGetURL(String urlString){
		byte[] imageData = null;
		try {
			// Use HttpPost instead of HttpPut
			HttpGet httpGet = new HttpGet(urlString);

			BasicHttpParams parameters = new BasicHttpParams();
			final int connectTimeoutSeconds = 40;
			final int socketTimeoutSeconds = 60;
			HttpConnectionParams.setConnectionTimeout(parameters,connectTimeoutSeconds * 1000);
			HttpConnectionParams.setSoTimeout(parameters,socketTimeoutSeconds * 1000);
			...
			if (responseEntity != null) {
				imageData = EntityUtils.toByteArray(responseEntity);
				return imageData;
			} else {
				return null;
			}
